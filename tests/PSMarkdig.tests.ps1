...
InModuleScope $moduleName {
    describe 'Get-MdDocument' {
      $testResourcesFolder = Join-Path -Path $PSScriptRoot -ChildPath 'testResources'
        it "Get document" {
            ...
            $testResult = Get-MdDocument -FilePath $file -ErrorAction Stop
            $testResult.GetType().FullName | should -be 'Markdig.Syntax.MarkdownDocument'
        }

        it "Get document using -Extensions=Yaml" {
          $file = Join-Path -Path $testResourcesFolder -ChildPath 'doc1.md'
          $testResult = Get-MdDocument -FilePath $file -Extension Yaml -ErrorAction Stop
          $testResult.GetType().FullName | should -be 'Markdig.Syntax.MarkdownDocument'
      }
    }
    ...
    describe 'Get-MdDocument' {
      ...
      it "Get Elements when Type=Markdig.Syntax.HeadingBlock" {
          $file = Join-Path -Path $testResourcesFolder -ChildPath 'doc1.md'
          $mdDocument = Get-MdDocument -FilePath $file -ErrorAction Stop
          $testResult = Get-MdElement -Document $mdDocument -Type 'Markdig.Syntax.HeadingBlock'
      }

      it "Get Elements when Type=Markdig.Syntax.Inlines.LinkInline" {
        $file = Join-Path -Path $testResourcesFolder -ChildPath 'doc1.md'
        $mdDocument = Get-MdDocument -FilePath $file -ErrorAction Stop
        $testResult = Get-MdElement -Document $mdDocument -Type 'Markdig.Syntax.Inlines.LinkInline'
      }

      it "Get Elements when Type=Markdig.Extensions.Yaml.YamlFrontMatterBlock" {
        $file = Join-Path -Path $testResourcesFolder -ChildPath 'doc1.md'
        $mdDocument = Get-MdDocument -FilePath $file -Extension Yaml -ErrorAction Stop
        $testResult = Get-MdElement -Document $mdDocument -Type 'Markdig.Extensions.Yaml.YamlFrontMatterBlock'
      }
    }
}